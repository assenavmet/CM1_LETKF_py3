MODULE module_restart
    ...
    ! 150820:  The following parameters allow users to include extra variables in 
    !          restart files.  Note that these variables are not usually needed 
    !          to restart CM1, but can be useful to have in restart files for 
    !          some purposes (eg, ensemble data assimilation).

    implicit none

    logical, parameter :: restart_file_theta  =  .true.  ! potential temperature
    logical, parameter :: restart_file_dbz    =  .true.  ! reflectivity

    logical, parameter :: restart_file_th0    =  .true.  ! base-state potential temperature
    logical, parameter :: restart_file_prs0   =  .true.  ! base-state pressure
    logical, parameter :: restart_file_pi0    =  .true.  ! base-state nondimensional pressure
    logical, parameter :: restart_file_rho0   =  .true.  ! base-state dry-air density
    logical, parameter :: restart_file_qv0    =  .true.  ! base-state water vapor mixing ratio
    logical, parameter :: restart_file_u0     =  .true.  ! base-state x-component velocity
    logical, parameter :: restart_file_v0     =  .true.  ! base-state y-component velocity

    logical, parameter :: restart_file_zs     =  .false.  ! terrain height
    logical, parameter :: restart_file_zh     =  .false.  ! height of half (scalar) grid points (3d array)
    logical, parameter :: restart_file_zf     =  .false.  ! height of full (w) grid points (3d array)

    logical, parameter :: restart_file_diags  =  .false.  ! theta and qv diags
                                                          ! (dissipative heating if output_dissheat=1)
                                                          ! (theta tndcy from microphys if output_mptend=1)
                                                          ! (hydrometeor fall vels if output_fallvel=1)
  ...
  ELSEIF( restart_format.eq.2 )THEN
    ...
    if( myid.eq.0 )then
      call     restart_prelim(nrst,ncid,mtime,xfref,yfref,zh,zf,sigma,sigmaf,  &
                              qname,num_soil_layers,nrad2d,dat2(1,1),dat2(1,2),dum1(ib,jb,kb))
    endif
...
! metadata:
  ...
  ELSEIF( restart_format.eq.2 )THEN
    IF(myid.eq.0)THEN
      ...
      call disp_err( nf90_put_var(ncid,varid,nstep) , .true. )

      call disp_err( nf90_inq_varid(ncid,"nrec",varid) , .true. )
      call disp_err( nf90_put_var(ncid,varid,nrec) , .true. )

      call disp_err( nf90_inq_varid(ncid,"prec",varid) , .true. )
      call disp_err( nf90_put_var(ncid,varid,prec) , .true. )

      call disp_err( nf90_inq_varid(ncid,"nwrite",varid) , .true. )
      call disp_err( nf90_put_var(ncid,varid,nwrite) , .true. )

      call disp_err( nf90_inq_varid(ncid,"nrst",varid) , .true. )
      call disp_err( nf90_put_var(ncid,varid,nrst) , .true. )

      call disp_err( nf90_inq_varid(ncid,"ndt",varid) , .true. )
      call disp_err( nf90_put_var(ncid,varid,ndt) , .true. )

      call disp_err( nf90_inq_varid(ncid,"old_format",varid) , .true. )
      call disp_err( nf90_put_var(ncid,varid,output_format) , .true. )

      call disp_err( nf90_inq_varid(ncid,"dt",varid) , .true. )
      call disp_err( nf90_put_var(ncid,varid,dt) , .true. )

      call disp_err( nf90_inq_varid(ncid,"dtlast",varid) , .true. )
      call disp_err( nf90_put_var(ncid,varid,dtlast) , .true. )

      call disp_err( nf90_inq_varid(ncid,"cflmax",varid) , .true. )
      call disp_err( nf90_put_var(ncid,varid,cflmax) , .true. )

      call disp_err( nf90_inq_varid(ncid,"mtime",varid) , .true. )
      call disp_err( nf90_put_var(ncid,varid,mtime) , .true. )

      call disp_err( nf90_inq_varid(ncid,"stattim",varid) , .true. )
      call disp_err( nf90_put_var(ncid,varid,stattim) , .true. )

      call disp_err( nf90_inq_varid(ncid,"taptim",varid) , .true. )
      call disp_err( nf90_put_var(ncid,varid,taptim) , .true. )

      call disp_err( nf90_inq_varid(ncid,"rsttim",varid) , .true. )
      call disp_err( nf90_put_var(ncid,varid,rsttim) , .true. )

      call disp_err( nf90_inq_varid(ncid,"radtim",varid) , .true. )
      call disp_err( nf90_put_var(ncid,varid,radtim) , .true. )

      call disp_err( nf90_inq_varid(ncid,"prcltim",varid) , .true. )
      call disp_err( nf90_put_var(ncid,varid,prcltim) , .true. )

      call disp_err( nf90_inq_varid(ncid,"adt",varid) , .true. )
      call disp_err( nf90_put_var(ncid,varid,adt) , .true. )

      call disp_err( nf90_inq_varid(ncid,"acfl",varid) , .true. )
      call disp_err( nf90_put_var(ncid,varid,acfl) , .true. )

      call disp_err( nf90_inq_varid(ncid,"dbldt",varid) , .true. )
      call disp_err( nf90_put_var(ncid,varid,dbldt) , .true. )

      call disp_err( nf90_inq_varid(ncid,"mass1",varid) , .true. )
      call disp_err( nf90_put_var(ncid,varid,mass1) , .true. )
...
! budget variables:
    ...
    IF( myid.eq.0 )THEN
      ...
      ELSEIF( restart_format.eq.2 )THEN
        ...
        call disp_err( nf90_put_var(ncid,varid,qbudget,(/1,1/),(/nbudget/)) , .true. )
        call disp_err( nf90_inq_varid(ncid,"asq",varid) , .true. )
        call disp_err( nf90_put_var(ncid,varid,asq,(/1,1/),(/numq/)) , .true. )
        call disp_err( nf90_inq_varid(ncid,"bsq",varid) , .true. )
        call disp_err( nf90_put_var(ncid,varid,bsq,(/1,1/),(/numq/)) , .true. )
      ...
      if(iptra.eq.1)then
        ...
        if(myid.eq.0)then
          ...
          elseif( restart_format.eq.2 )then
            ...
            call disp_err( nf90_put_var(ncid,varid,npt) , .true. )
...
162         format(i2.2)
        ...
        if(myid.eq.0)then
          ...
          elseif( restart_format.eq.2 )then
            call disp_err( nf90_inq_varid(ncid,"npt",varid) , .true. )
            call disp_err( nf90_put_var(ncid,varid,nvar) , .true. )
      ...
      if(iprcl.eq.1)then
        ...
        if(myid.eq.0)then
          ...
          elseif( restart_format.eq.2 )then
            ...
            call disp_err( nf90_put_var(ncid,varid,nparcels) , .true. )
        ...
        if(myid.eq.0)then
          ...
          elseif( restart_format.eq.2 )then
            ...
            n = 3
            call disp_err( nf90_put_var(ncid,varid,ploc,(/1,1/),(/n,nparcels/)) , .true. )
      ...
      if( wbc.eq.2 )then
        ...
        do k=1,nk
          ...
          if( myid.eq.0 )then
            ...
            elseif( restart_format.eq.2 )then
              ncstatus = nf90_put_var(ncid,varid,dumy,(/1,k/),(/ny,1/))
              if(ncstatus.ne.nf90_noerr)then
      ...
      if( sbc.eq.2 )then
        ...
        do k=1,nk
          ...
          if( myid.eq.0 )then
            ...
            elseif( restart_format.eq.2 )then
              ncstatus = nf90_put_var(ncid,varid,dumx,(/1,k/),(/nx,1/))
              if(ncstatus.ne.nf90_noerr)then
...
! metadata:
  ...
  ELSEIF( restart_format.eq.2 )THEN
    IF(myid.eq.0)THEN
      ...
      call disp_err( nf90_inq_varid(ncid,"nstep",varid) , .true. )
      call disp_err( nf90_get_var(ncid,varid,nstep) , .true. )

      call disp_err( nf90_inq_varid(ncid,"nrec",varid) , .true. )
      call disp_err( nf90_get_var(ncid,varid,nrec) , .true. )

      call disp_err( nf90_inq_varid(ncid,"prec",varid) , .true. )
      call disp_err( nf90_get_var(ncid,varid,prec) , .true. )

      call disp_err( nf90_inq_varid(ncid,"nwrite",varid) , .true. )
      call disp_err( nf90_get_var(ncid,varid,nwrite) , .true. )

      call disp_err( nf90_inq_varid(ncid,"nrst",varid) , .true. )
      call disp_err( nf90_get_var(ncid,varid,nrst) , .true. )

      call disp_err( nf90_inq_varid(ncid,"ndt",varid) , .true. )
      call disp_err( nf90_get_var(ncid,varid,ndt) , .true. )

      call disp_err( nf90_inq_varid(ncid,"old_format",varid) , .true. )
      call disp_err( nf90_get_var(ncid,varid,old_format) , .true. )

      call disp_err( nf90_inq_varid(ncid,"dt",varid) , .true. )
      call disp_err( nf90_get_var(ncid,varid,dt) , .true. )

      call disp_err( nf90_inq_varid(ncid,"dtlast",varid) , .true. )
      call disp_err( nf90_get_var(ncid,varid,dtlast) , .true. )

      call disp_err( nf90_inq_varid(ncid,"cflmax",varid) , .true. )
      call disp_err( nf90_get_var(ncid,varid,cflmax) , .true. )

      call disp_err( nf90_inq_varid(ncid,"mtime",varid) , .true. )
      call disp_err( nf90_get_var(ncid,varid,mtime) , .true. )

      call disp_err( nf90_inq_varid(ncid,"stattim",varid) , .true. )
      call disp_err( nf90_get_var(ncid,varid,stattim) , .true. )

      call disp_err( nf90_inq_varid(ncid,"taptim",varid) , .true. )
      call disp_err( nf90_get_var(ncid,varid,taptim) , .true. )

      call disp_err( nf90_inq_varid(ncid,"rsttim",varid) , .true. )
      call disp_err( nf90_get_var(ncid,varid,rsttim) , .true. )

      call disp_err( nf90_inq_varid(ncid,"radtim",varid) , .true. )
      call disp_err( nf90_get_var(ncid,varid,radtim) , .true. )

      call disp_err( nf90_inq_varid(ncid,"prcltim",varid) , .true. )
      call disp_err( nf90_get_var(ncid,varid,prcltim) , .true. )

      call disp_err( nf90_inq_varid(ncid,"adt",varid) , .true. )
      call disp_err( nf90_get_var(ncid,varid,adt) , .true. )

      call disp_err( nf90_inq_varid(ncid,"acfl",varid) , .true. )
      call disp_err( nf90_get_var(ncid,varid,acfl) , .true. )

      call disp_err( nf90_inq_varid(ncid,"dbldt",varid) , .true. )
      call disp_err( nf90_get_var(ncid,varid,dbldt) , .true. )

      call disp_err( nf90_inq_varid(ncid,"mass1",varid) , .true. )
      call disp_err( nf90_get_var(ncid,varid,mass1) , .true. )
...
! budget variables:
    ...
    IF( myid.eq.0 )THEN
      ...
      ELSEIF( restart_format.eq.2 )THEN
        call disp_err( nf90_inq_varid(ncid,"qbudget",varid) , .true. )
        call disp_err( nf90_get_var(ncid,varid,qbudget,(/1,1/),(/nbudget/)) , .true. )
        call disp_err( nf90_inq_varid(ncid,"asq",varid) , .true. )
        call disp_err( nf90_get_var(ncid,varid,asq,(/1,1/),(/numq/)) , .true. )
        call disp_err( nf90_inq_varid(ncid,"bsq",varid) , .true. )
        call disp_err( nf90_get_var(ncid,varid,bsq,(/1,1/),(/numq/)) , .true. )
...
!  passive tracers:
      ...
      if( myid.eq.0 )then
        ...
        elseif( restart_format.eq.2 )then
          call disp_err( nf90_inq_varid(ncid,"npt",varid) , .true. )
          call disp_err( nf90_get_var(ncid,varid,nvar) , .true. )
...
162             format(i2.2)
          ...
          if( nread .lt. nvar )then
            ...
            do n=nread+1,nvar
              call  readr(ni,nj,1,nk,nx,ny,dum1(ib,jb,1),'pta     ',       &
                          ni,nj,ngxy,myid,numprocs,nodex,nodey,orecs,51,   &
                          ncid,time_index,restart_format,restart_filetype, &
                          dat1(1,1),dat2(1,1),dat3(1,1,1),reqt,ppnode,d3n,d3t,mynode,nodemaster,nodes,d2is,d2js,d3is,d3js)
            enddo
          ...
          if( myid.eq.0 )then
            IF( nvar.eq.nparcels )THEN
              ...
              elseif( restart_format.eq.2 )then
                call disp_err( nf90_inq_varid(ncid,"ploc",varid) , .true. )
                n = 3
                call disp_err( nf90_get_var(ncid,varid,ploc,(/1,1/),(/n,nparcels/)) , .true. )
            ...
            ELSE
              ...
              elseif( restart_format.eq.2 )then
                call disp_err( nf90_inq_varid(ncid,"ploc",varid) , .true. )
                n = 3
                call disp_err( nf90_get_var(ncid,varid,pfoo,(/1,1/),(/n,nvar/)) , .true. )
      ...
      if( wbc.eq.2 )then
        ...
        do k=1,nk
          if( myid.eq.0 )then
            ...
            elseif( restart_format.eq.2 )then
              ncstatus = nf90_get_var(ncid,varid,dumy,(/1,k/),(/ny,1/))
              if(ncstatus.ne.nf90_noerr)then
      ...
      if( sbc.eq.2 )then
        ...
        do k=1,nk
          if( myid.eq.0 )then
            ...
            elseif( restart_format.eq.2 )then
              ncstatus = nf90_get_var(ncid,varid,dumx,(/1,k/),(/nx,1/))
              if(ncstatus.ne.nf90_noerr)then
...
!-------------------------------------------------------------------------------
!ccccc  restart_filetype =1,  one output file  ccccccccccccccccccccccccccccccccc
!-------------------------------------------------------------------------------

    rf1:  IF( restart_filetype.eq.1 )THEN
        ...
        IF( myid.eq.msk )THEN
          ...
          ELSEIF( restart_format.eq.2 )THEN
            ...
            if(numk1.eq.numk2)then
              status = nf90_put_var(ncid,varid,dat2,(/1,1/),(/nxr,nyr/))
            else
              status = nf90_put_var(ncid,varid,dat2,(/1,1,k/),(/nxr,nyr,1/))
            endif
            if(status.ne.nf90_noerr)then
...
!-------------------------------------------------------------------------------
    ...
    kloop:  DO k=numk1,numk2
          ...
          ELSEIF( restart_format.eq.2 )THEN
            ! ----- netcdf format -----
            if(numk1.eq.numk2)then
              status = nf90_get_var(ncid,varid,dat2,(/1,1/),(/nxr,nyr/))
            else
              status = nf90_get_var(ncid,varid,dat2,(/1,1,k/),(/nxr,nyr,1/))
            endif
            if(status.ne.nf90_noerr)then
