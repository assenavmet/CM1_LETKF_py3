...
#ref_cmap = colors.LinearSegmentedColormap('REFcmap', _NWSRef_data, 64)
ref_cmap = ctables.REF_default
...
def chk_pyDart_version(h5file, verbose = True):
    ...
    if checked_file_version and h5file.title != version_string:
        ...
        if not checked_file_version and verbose == True:
            print(("\n  pyDART file is version:  %s " % h5file.title))
            print("  \/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\n")
            return
...
#                      user's observation type            kind   DART official name
      ...
      Look_Up_Table={ "DOPPLER_VELOCITY":                 [11,   "DOPPLER_VELOCITY"] ,
                      "DOPPLER_RADIAL_VELOCITY":          [11,   "DOPPLER_VELOCITY"] ,
                      "REFLECTIVITY":                     [12,   "REFLECTIVITY"],
                      "RADAR_REFLECTIVITY":               [12,   "REFLECTIVITY"],
...
# Construct a variable to search table
        ...
        if start != None and type(start) == tuple:
            ...
            if self.verbose:
                if self.debug:  print(("  pyDART.search:  Start datetime:  %s" % self.start))
        
        if end != None and type(end) == tuple:
            self.end = py_datetime(end[0],end[1],end[2],end[3],end[4],end[5])
            if self.verbose:
                if self.debug:  print(("  pyDART.search:  End datetime:  %s" % self.start))
...
# A quick routine to grid pyDART data
    ...
    def grid(self, xo, yo, zo, dx=None, dy=None, fill_value=0.0):
...
#       zi     = rbf_z(xx,yy)
#
        zi = interpolate.griddata((xo, yo), zo, (xi[None,:], yi[:,None]), method='linear',fill_value=fill_value)
...
# Now, call grid to grid some data
        ...
        if variable.upper() == "DBZ" or variable.upper() == "REFLECTIVITY":
            ...
            plt  = map.contourf(xx, yy, zimask, clevels, cmap=ref_cmap)
            cbar = map.colorbar(plt,location='bottom',pad=0.40)
            cbar.ax.set_title("dBZ")
            P.title("Reflectivity at   %s  EL:  %5.2f" % (time, el))
            ...
            plt = map.contourf(xx,yy,zi, clevels, cmap=ref_cmap)
            cbar = map.colorbar(plt,location='bottom',pad=0.40)
            cbar.ax.set_title("Clear Air Reflecivity")
            P.title("0-Reflectivity at   %s  EL:  %5.2f" % (time, el))
            
        else:
            amin1, amax1, clevels = nice_mxmnintvl(-50, 50.)
...
# Plot MRMS data
    ...
    def plot_MRMS(self, start=None, end=None, condition=None, savefig=None):

# Return some data specified (hopefully) by user search
# Later, we might add an error check here to make sure the data are all the same kind

        self.search(variable="CLEARAIR_REFLECTIVITY", start=start, end=end, condition=condition)
        clear_air = self.get_data()   

        if clear_air != -1:
            x0 = clear_air['lon'][:]
            y0 = clear_air['lat'][:]      
            x0 =  N.where(x0 > 180., x0 - 360., x0)  # make west longitudes...
            z0 =  clear_air['value'][:]
            time = clear_air['date'][0]
            el = clear_air['z'][0]

        self.search(variable="REFLECTIVITY", start=start, end=end, condition=condition)
        dbz  = self.get_data()

        if dbz != -1:
            x1 = dbz['lon'][:]
            y1 = dbz['lat'][:]       
            x1 = N.where(x1 > 180., x1 - 360., x1)  # make west longitudes...
            z1 = dbz['value'][:]
            el = dbz['z'][0]
            time = dbz['date'][0]
...
# Create Map
        ...
        if dbz == -1 and clear_air == -1:
            print("\nplotMRMS:  No data found for clear air or DBZ, exiting...\n")
            return
        else:
            fig = P.figure(figsize = (16,10))
            ax = fig.add_subplot(111, aspect='equal')
            ...
            map.drawstates(linewidth=0.5, color='k')
    ...
    # Plot clear air    
            if clear_air != -1:  
                xx, yy = list(map(x0, y0))
                plot = map.scatter(xx, yy, c="black", marker="o", ax=ax, alpha=0.3, edgecolors='none')

    # Plot DBZ    
            if dbz != -1: 
                xx, yy = list(map(x1, y1))
                plot = map.scatter(xx, yy, c=z1, cmap=ref_cmap, marker=None, verts=dbz_verts, vmin=0, vmax = 70., ax=ax, edgecolors='none')
        
                cbar = map.colorbar(plot, location = 'right', pad = '3%')
                cbar.ax.set_title("dBZ")
                P.title("Reflectivity at   %s\nHGT:  %5.2f km" % (time, el/1000.))
                
            if savefig == None:
                P.savefig("pyDART_MRMS.png")
            else:
                P.savegif(savefig+".png")
...
#-------------------------------------------------------------------------------
    ...
    def get_data(self, variable=None, all=False):
        ...
        if len(self.index) == 0 and all ==False:
            if self.verbose: print("pyDART.get_data:  No search indices supplied, no data...")
            return -1
...
# Create group for misc file header information
        ...
        num_copies = int(stuff[1])         # Important, if you have a truth file, you need to know this number (either 1 or 2)
        num_qc     = int(stuff[3])         
        
        stuff       = fi.readline()         # Read(str) "num_obs" line
        stuff       = stuff.split()
        num_obs     = int(stuff[1])
        max_num_obs = int(stuff[3])
...
# Read in the obs sequentially!
        ...
        while n < group_header.attributes.cols.num_obs[0]:
            ...
            stuff            = fi.readline()
            stuff            = stuff.split()
            row["previous"]  = int(stuff[0])
            row["next"]      = int(stuff[1])
            row["cov_group"] = int(stuff[2])
            ...
            if len(stuff) == 4:
                row["vert_coord"] = int(stuff[3])
            else:
                row["vert_coord"] = int(fi.readline())
...
# Check to see if its radial velocity and add platform information...need BETTER CHECK HERE!
            ...
            if row["kind"] == ObType_LookUp("VR"):
                ...
                if len(stuff) == 4:
                    row["platform_vert_coord"] = int(stuff[3])
                else:
                    row["platform_vert_coord"] = int(fi.readline())      
                fi.readline()
                ...
                row['platform_key']      = int(fi.readline())
...
# Need to switch the lat and lon these around to be correct in version 2 of pyDART...
            ...
            if row['kind'] == ObType_LookUp("DBZ"):    # See if there is information about the azimuth and elevation supplied
                ...
                if 'azimuth' in list(obs_dict.keys()):
                    row['azimuth'] = obs_dict['azimuth'][n]
                    if row['azimuth'] < 0: row['azimuth'] = row['azimuth'] + 360.

                if 'elevation' in list(obs_dict.keys()):
...
class DART_ob_kinds(IsDescription):
        index               = Int64Col(dflt=int(_missing))
        name                = StringCol(255)

class DART_header(IsDescription):
        origin_file         = StringCol(255)
        num_copies          = Int64Col(dflt=int(_missing))
        num_qc              = Int64Col(dflt=int(_missing))
        num_obs             = Int64Col(dflt=int(_missing))
        max_num_obs         = Int64Col(dflt=int(_missing))
        first               = Int64Col(dflt=int(_missing))
        last                = Int64Col(dflt=int(_missing))
...
class DART_obs(IsDescription):
        ...
        kind                = Int64Col  (dflt=int(_missing))
        elevation           = Float64Col(dflt=_missing)
        azimuth             = Float64Col(dflt=_missing)
        error_var           = Float64Col(dflt=_missing)
        seconds             = Int64Col  (dflt=int(_missing))
        days                = Int64Col  (dflt=int(_missing))
        date                = StringCol (25)
        utime               = Int64Col  (dflt=int(_missing))
        index               = Int64Col  (dflt=int(_missing))
        ...
        platform_vert_coord = Int64Col  (dflt=int(_missing))
        ...
        platform_key        = Int64Col  (dflt=int(_missing))
...
# Command line interface for PyDart
    ...
    parser.add_option(      "--plotMRMS",    dest="plotMRMS",  default=False, help = "Plot MRMS data", action="store_true")
    ...
    if options.plotMRMS:
      ...
      if myDART.verbose:  print("\n PyDart:  plotting data")
      myDART.file(filename = options.file)
      myDART.plot_MRMS(start = start, end = end, condition=options.condition, savefig=options.output)
      print()
