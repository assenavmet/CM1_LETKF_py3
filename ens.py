...
#===============================================================================
#   
def mymap_draw_shapes(bmap, shape_env=None, ax=None):
...
#===============================================================================
  ...
  if counties:
    mymap_draw_shapes(map, shape_env=counties, ax=ax)
...
#===============================================================================
  ...
  PLOT_ONE(mfld, x2d, y2d, map, cint=0.2, height=ens.zc[klevel], label = "Mean P-Theta", ax = ax1, counties=shapefiles, sym=True)

  mfld = 1000.*(ens['QV'][:,klevel,:,:].mean(axis=0) - ens['QV'][:,klevel,-1,-1].mean(axis=0))
  PLOT_ONE(mfld, x2d, y2d, map, cint=0.25, height=ens.zc[klevel],label = "Mean P-QV", ax = ax2, counties=shapefiles, sym=True)

  mfld = ens['U'][:,klevel,:,:].mean(axis=0)
  PLOT_ONE(mfld, x2d, y2d, map, cint=1.0, height=ens.zc[klevel],label = "Mean U-Velo", ax = ax3, counties=shapefiles, sym=True)

  mfld = ens['V'][:,klevel,:,:].mean(axis=0)
  PLOT_ONE(mfld, x2d, y2d, map, cint=1.0, height=ens.zc[klevel],label = "Mean V-Velo", ax = ax4, counties=shapefiles, sym=True)

  mfld = ens['W'][:,klevel,:,:].mean(axis=0)
  PLOT_ONE(mfld, x2d, y2d, map, cint=0.5, height=ens.zc[klevel],label = "Mean W-Velo", ax = ax5, counties=shapefiles, sym=True)

# Standard deviations...

  mfld = ens['TH'][:,klevel,:,:].std(axis=0)
  PLOT_ONE(mfld, x2d, y2d, map, cint=0.25, height=ens.zc[klevel],label = "Theta Stdev", ax = ax6, counties=shapefiles)
  
  mfld = 1000.*ens['QV'][:,klevel,:,:].std(axis=0)
  PLOT_ONE(mfld, x2d, y2d, map, cint=0.25, height=ens.zc[klevel],label = "QV Stdev", ax = ax7, counties=shapefiles)

  mfld = ens['U'][:,klevel,:,:].std(axis=0)
  PLOT_ONE(mfld, x2d, y2d, map, cint=0.5, height=ens.zc[klevel],label = "U Stdev", ax = ax8, counties=shapefiles)

  mfld = ens['V'][:,klevel,:,:].std(axis=0)
  PLOT_ONE(mfld, x2d, y2d, map, cint=0.5, height=ens.zc[klevel],label = "V Stdev", ax = ax9, counties=shapefiles)

  mfld = ens['W'][:,klevel,:,:].std(axis=0)
  PLOT_ONE(mfld, x2d, y2d, map, cint=0.25, height=ens.zc[klevel], label = "W Stdev", ax = ax10, counties=shapefiles)
...
#===============================================================================
#
def ens_PLOT_9PANEL(ens, klevel = 4, obs=False, savefig=None, cparams = None, var='DBZ', 
                    zoom = None, **kwargs):
#
#===============================================================================
  ...
  for n, ax in enumerate(ax_grid.ravel()):
    ...
    if n == 4 and obs:
      ...
      if var == "DBZ":
          ...
          PLOT_ONE(mfld, x2d, y2d, map, clevels=clevels, height=ens.zc[klevel], label = label, ax = ax, \
               counties=shapefiles, cmap=ctables.REF_default, zoom = zoom)
      else:
          if var == "THP":
          ...
          PLOT_ONE(mfld, x2d, y2d, map, clevels=clevels, height=ens.zc[klevel], label = label, ax = ax, \
               counties=shapefiles, cmap=ens_default_cmap, zoom = zoom)
