...
_prior_files = "Prior_*"
_plotfilename = "VR_Innovations"

_cmin, _cmax = -10., 10.

variable = "VR"   # valid variables:  VR, REFL
...
if __name__ == "__main__":
    ...
    IN_TZ = N.zeros((zbins.size,nbins))
    IN_T  = N.zeros((nbins))
    IN_Z  = N.zeros((zbins.size))
    m = -1
    
    fig = P.figure(figsize=(12,12))
    fig.text(0.72, 0.72, "\n\nInnovations\n\n", size=20, va="baseline", ha="center", multialignment="center")
    fig.text(0.72, 0.65, plotfilename, size=16, va="baseline", ha="center", multialignment="center")

    print("\n 2D INNOVATION CALCULATIONS......\n")
...
# Plotting
    ...
    if auto_clevels:
        cmin, cmax, cint, clevels = nice_clevels(-10., 10, outside=False, cint = 1.0)
    else:
...
# time series Innovations
    ...
    print("\n TIME-SERIES INNOVATION CALCULATIONS......\n")
...
# Height Innovations
    ...
    print("\n ZBIN-D INNOVATION CALCULATIONS......\n")
...
#   cmin, cmax, cint = nice_mxmnintvl(CR_Z.min(), CR_Z.max(), outside=True, cint=1.0)  # Use this to get limits of the plot
    ...
    axY.plot(IN_Z, zbins/1000., lw=2.0, color='k')
    axY.set_ylim(0.0,zbins.max()/1000.)
    axY.set_xlim(_cmin,_cmax)
    axY.set_yticklabels([])
    axY.set_xlabel("Innovations")
    axY.grid(True)
